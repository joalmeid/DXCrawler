...
function checkVersion(library, version) {
    ...
    if (!vinfo.needsUpdate && library.minVersions.length > 0) {
        // Checking against first minimal version
        var minMinVersion = library.minVersions[0].major;
        var minMinVersionParts = minMinVersion.match(/(\d+)\.(\d+)\./);
        var versionParts = version.match(/(\d+)\.(\d+)\./);
        
        if (versionParts && versionParts.length > 1 && minMinVersionParts[1] > versionParts[1]) {
            vinfo.minVersion = library.minVersions[0].major + library.minVersions[0].minor;
